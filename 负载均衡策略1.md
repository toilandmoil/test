...
```mermaid
graph TB
	将IPv4按照网段分类-->处理待分配网段1-网段n的IPv4-->检查网段1-网段n的IPv4是否均衡-->检查IPv4地址是否均衡-->检查IPv6地址是否均衡-->检查IP总数是否均衡
```
...
```mermaid
graph LR
	待分配IP-->|网段1|计算各pnn节点的IP属于网段1的IP数量-->|ip arry|计算IP数量最少的pnn-->|pnn|IP分配到pnn上
```
...
```c
/*
* 检查各网段ip是否均衡，不均衡会调整ip在主机节点的分配，直到均衡
*/
